    ...
    def run(self):
        ...
        for w_i, w_j in self.pool_pair_indices:
            ...
            for shift in np.ndindex(2 * self.Rx + 1, 2 * self.Ry + 1, 2 * self.Rz + 1):
                ...
                local_coulomb_potential = self.solve_poisson(
                    wf_i.conj() * wf_shifted_j, global_potential=False
                )
                V[shift[0], shift[1], shift[2], w_i, w_j] = (
                    np.vdot(wf_i.conj() * wf_shifted_j, local_coulomb_potential)
                    * self.GD.dv
                )
